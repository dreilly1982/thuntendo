...
use ringbuf::{LocalRb, Producer, SharedRb};
...
impl<'a> NES<'a> {
    ...
    pub fn clock(&self, frame: &mut[u8], sys_counter: &mut u32, producer: &mut Producer<f32, Arc<SharedRb<f32, Vec<MaybeUninit<f32>>>>>) -> bool {
        ...
        *self.audio_cycles.borrow_mut() += 1;
        if *self.audio_cycles.borrow() >= *self.cycles_per_sample.borrow() {
            let end = Instant::now();
            if let Some(apu) = self.apu.borrow().as_ref() {
                while producer.push(apu.borrow_mut().get_output_sample()).is_err() {
    ...
    pub fn set_sample_frequency(&self, sample_rate: u32) {
        let cycles_per_sample = NTSC_CRYSTAL_FREQUENCY as u32 / sample_rate;
        *self.cycles_per_sample.borrow_mut() = cycles_per_sample;
        *self.audio_cycles.borrow_mut() = 0;
    }
}
